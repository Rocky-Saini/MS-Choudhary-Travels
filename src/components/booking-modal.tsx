...
import { X, User, Phone, MapPin, Users, CreditCard, CheckCircle, Clock } from 'lucide-react'
    ...
    try {
      ...
      if (data.success) {
        if (!data.requiresPayment) {
          // Direct booking — needs admin approval (no advance)
          setBookingCode(data.booking.bookingCode)
          setWhatsappLink(data.whatsappLink || '')
          setPendingApproval(data.pendingApproval === true)
          setStep('success')
        } else {
                ...
                <div className="text-center py-8">
                  {pendingApproval ? (
                    <>
                      ...
                      <p className="text-gray-600 mb-2">Aapki booking request mil gayi hai. Hamari team aapko call ya WhatsApp par confirm karegi.</p>
                      {bookingCode && (
                        <p className="text-sm text-amber-600 font-mono bg-amber-50 px-3 py-1 rounded-lg inline-block mb-4">
                          Code: {bookingCode}
                        </p>
                      )}
                      <p className="text-sm text-gray-500 mb-4">
                        Seat confirm hone par aapko message milega. Tab tak seat block nahi hoti.
                      </p>
                      <div className="bg-amber-50/60 rounded-xl p-3 mb-4 text-sm text-gray-600">
                        Jaldi confirmation ke liye call karein:{' '}
                        <a href="tel:+917830673603" className="font-medium text-indigo-600">+91 7830673603 (Ajeem)</a>
                      </div>
