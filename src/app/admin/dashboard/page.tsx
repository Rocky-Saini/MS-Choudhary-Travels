...
function buildVerifyMessage(b: BookingData): string {
  const t = b.trip
  let rideDetails = ''
  if (t) {
    const tripDate = new Date(t.date).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata' })
    const departureTime = new Date(t.departureTime).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true, timeZone: 'Asia/Kolkata' })
    rideDetails = `🚗 Vehicle: ${t.vehicle.vehicleNumber}
👤 Driver: ${t.driver.name} (${t.driver.mobile})
📍 Route: ${t.route.origin} → ${t.route.destination}
📅 Date: ${tripDate}
🕐 Time: ${departureTime}
`
  }
  return `🙏 *Booking Verification - MS Choudhary Travels*

Hello ${b.customerName}!

Aapki booking request mili hai. Confirm karne se pehle verify kar rahe hain:

${rideDetails}💺 Seats: ${b.seats}
💰 Fare: ₹${b.totalFare} (pay to driver)
...
❓ Kya aap is ride me travel karna chahte hain? Reply karein YES.
Confirm hone par aapko full details bhej denge.
...
interface BookingData { id: string; tripId: string; customerName: string; customerMobile: string; pickupPoint: string; dropPoint: string; seats: number; totalFare: number; advancePaid: number; remainingFare: number; status: string; bookingCode: string; createdAt: string; trip?: { departureTime: string; date: string; vehicle: { vehicleNumber: string }; driver: { name: string; mobile: string }; route: { origin: string; destination: string } } }
...
export default function AdminDashboard() {
  ...
  const [confirmWhatsAppLink, setConfirmWhatsAppLink] = useState('')
  // Confirm + Assign a pending booking to a chosen cab
  ...
  const confirmAssign = async (tripId: string) => {
    ...
    const res = await fetch('/api/admin/confirm-booking', {
      method: 'POST', headers: headers(),
      body: JSON.stringify({ bookingId: assigningBooking.id, tripId }),
    })
    const data = await res.json()
    setAssignTripId(null)
    if (data.success) {
      setAssigningBooking(null)
      setConfirmWhatsAppLink(data.whatsappLink || '')
      fetchBookings()
      fetchDashboard()
    } else {
      alert(data.error || 'Failed to confirm')
  ...
  const updateBookingStatus = async (id: string, status: string) => {
    ...
    if (status === 'CANCELLED' && data.whatsappLink) {
      setCancelWhatsAppLink(data.whatsappLink)
    }
    if (status === 'CONFIRMED' && data.whatsappLink) {
      setConfirmWhatsAppLink(data.whatsappLink)
    }
    fetchBookings()
    fetchDashboard()
      ...
      <div className="lg:ml-64 p-4 md:p-6 overflow-x-hidden">
        ...
        {!pageLoading && activeTab === 'bookings' && (
          <div className="space-y-6">
              ...
              {bookings
                ...
                <Card key={b.id}><CardContent className="p-4 flex items-start justify-between flex-wrap gap-3">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <Badge variant={b.status === 'CONFIRMED' ? 'secondary' : b.status === 'CANCELLED' ? 'destructive' : b.status === 'COMPLETED' ? 'default' : 'warning'}>{b.status === 'PENDING' ? 'NEEDS APPROVAL' : b.status}</Badge>
                    {b.status === 'PENDING' && <>
                      <a href={`tel:+91${b.customerMobile}`} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-blue-50 text-blue-600 text-xs font-medium hover:bg-blue-100"><Phone className="w-3 h-3" />Call</a>
                      <a href={`https://wa.me/91${b.customerMobile}?text=${encodeURIComponent(buildVerifyMessage(b))}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-green-50 text-green-600 text-xs font-medium hover:bg-green-100"><svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>Verify</a>
                      <Button size="sm" variant="secondary" onClick={() => openConfirmAssign(b)}>✓ Confirm + Assign</Button>
                      <Button size="sm" variant="destructive" onClick={() => updateBookingStatus(b.id, 'CANCELLED')}>✗ Reject</Button>
                    </>}
            ...
            {assigningBooking && (
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setAssigningBooking(null)} />
                <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto p-6">
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="space-y-2">
                        {sorted.map(t => {
                          const isOriginal = t.id === origTripId
                          const canFit = t.availableSeats >= assigningBooking.seats && t.status !== 'COMPLETED' && t.status !== 'CANCELLED'
                          return (
                            <div key={t.id} className={`p-4 rounded-xl border-2 ${isOriginal ? 'border-indigo-500 bg-indigo-50/60' : 'border-gray-200'}`}>
                              <div className="flex items-center justify-between flex-wrap gap-2">
                                <div>
                                  <p className="font-bold text-gray-900 flex items-center gap-2">
                                    <Car className="w-4 h-4 text-indigo-600" />{t.vehicleNumber}
                                    {isOriginal && <span className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-600 text-white font-medium">Booked Cab</span>}
                                  </p>
                                  <p className="text-xs text-gray-500">{t.route} • {t.departureTime} • {t.driverName}</p>
                                  <p className={`text-xs font-medium mt-0.5 ${t.availableSeats > 0 ? 'text-emerald-600' : 'text-red-500'}`}>{t.availableSeats}/{t.totalSeats} seats available • {t.status}</p>
                                </div>
                                <button onClick={() => confirmAssign(t.id)} disabled={!canFit || assignTripId === t.id}
                                  className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-xl disabled:opacity-40 disabled:cursor-not-allowed">
                                  {assignTripId === t.id ? '⏳...' : canFit ? 'Assign' : 'Full'}
                                </button>
                              </div>
                            </div>
                          )
            ...
            {confirmWhatsAppLink && (
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setConfirmWhatsAppLink('')} />
                <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 text-center">
                  ...
                  <p className="text-sm text-gray-500 mb-4">Seat assign ho gayi. Customer ko confirmation bhejein:</p>
                  <a href={confirmWhatsAppLink} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-6 py-3 bg-green-500 hover:bg-green-600 text-white font-medium rounded-xl shadow-lg">
                    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                    Send Confirmation on WhatsApp
                  </a>
                  <div className="mt-4"><button onClick={() => setConfirmWhatsAppLink('')} className="text-sm text-gray-500 hover:text-gray-700">Close</button></div>
                </div>
              </div>
            )}
